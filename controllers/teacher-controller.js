...
const teacherRegister = async (req, res) => {
  ...
  try {
    ...
    // existingTeacherByEmail.teachSubject.push(teachSubject);
    ...
    //     return res.send({ message: 'Subjects added to existing teacher' });
    // }
    // else {
    //     // let result = await teacher.save();
    //     // await Subject.findByIdAndUpdate(teachSubject, { teacher: teacher._id });
    ...
    //     return res.send({ message: 'New teacher created with subjects' });
...
const addCOSForSubject = async (req, res) => {
  try {
    ...
    // if (subject[0]) {
    //   // Assuming req.body.courseOutcomes is an array of COs
    //   subject[0].courseOutcomes.push(...CO);

    //   // Save the updated subject with the new courseOutcomes
